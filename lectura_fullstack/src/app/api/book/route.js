...
export async function GET() {
    const { data: libro, error } = await supabase.from("libro").select("*");
...
export async function PUT(request){
    ...
    if(error){
        return new Response(
            JSON.stringify({message: "El Libro no se ha podido actualizar"}),
            { headers: { "Content-Type": "application/json" } }
        );
    }

    return new Response(
        JSON.stringify({message: "Libro actualizado correctamente"}),
        { headers: { "Content-Type": "application/json" } }
    );
}  
